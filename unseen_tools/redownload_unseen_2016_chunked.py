...
OUT = Path(r"D:\oro_top40_shortmem_OLD\unseen_2016_retry")
SYMBOL = "XAUUSD"
START = pd.Timestamp("2015-12-01T00:00:00Z")
END = pd.Timestamp("2017-01-05T23:59:00Z")
TIMEFRAMES = {
...
def download_tf(name: str, timeframe: int, expected_minutes: int) -> pd.DataFrame:
    ...
    # Validate actual intraday spacing on consecutive bars. Weekend/session gaps are ignored.
...
def main() -> None:
    ...
    manifest = {
        ...
        "method": "monthly_chunked_copy_rates_range_with_timeframe_validation",
        "files": {},
    }
    ...
    try:
        for name, (tf, minutes) in TIMEFRAMES.items():
            ...
            path = OUT / f"XAUUSD_{name}_UNSEEN_2016.csv"
    ...
    out_manifest = OUT / "UNSEEN_2016_RETRY_MANIFEST.json"
    out_manifest.write_text(json.dumps(manifest, indent=2), encoding="utf-8")
    print("\nUNSEEN_2016_CHUNKED_DOWNLOAD=PASS")
